...
> 当前仓库正在从“纯 CLI 原型”升级为 **“组织智能内核 + Skill 交付层”** 的项目结构。CLI 继续保留为调试/演示控制台，Skill 层则承接赛事提交和后续飞书落地。
...
当前版本已经实现一个基于 **Node.js / TypeScript** 的组织智能内核，并新增了面向赛事提交的 **Skill 包装层**，当前既可作为 CLI 运行，也可按 Skill 方式理解与演示。
...
1. **Intelligence Core（组织智能内核）**：保留现有 TypeScript 分析能力
2. **Skill Packaging Layer（Skill 交付层）**：新增 `skill/` 目录，承接赛事提交所需的能力定义、参考说明和适配脚本

当前分支新增：

- `skill/SKILL.md`：定义项目作为组织智能 Skill 的定位、适用场景、输入输出和演进方向
- `skill/references/submission-positioning.md`：统一赛事提交时的叙事口径
- `skill/references/feishu-output-plan.md`：定义结果如何回流到 Feishu Docs / Base / Chat
- `skill/scripts/run-skill.mjs`：最小 Skill 运行入口
- `skill/scripts/export-doc.mjs`：最小 Feishu Docs 输出模拟脚本
